...
# endif
  ...
  ADCSRA &= 0xf8;  // remove bits set by Arduino library (last 3 bit)
  ADCSRA |= 0x06;
  ...
  noInterrupts();
  TCCR1A = 0; // clear control registers (Timer/Counter1 Control Register A)
  TCCR1B = 0;
...
#endif
  // Holds reference voltage value and the analog pin to select
  //
  //  7 bit 6 bit 5 bit 4 bit 3 bit 2 bit 1 bit 0 bit
  //  REFS1 REFS0 ADLAR       MUX3 ...          MUX0
  //
  //      0     0  AREF, Internal Vref turned off
  //      0     1  AVcc with external capacitor on AREF pin
  //      1     1  Internal 1.1 V (Atmega168/328) or 2.56 V (Atmega8)

  //                              0     0     0     0  ADC0
  //                              0     1     0     1  ADC5
  ADMUX = _BV(REFS0) | CT1PIN; // start ADC conversion for first current, ADLAR=0
  ADCSRA |= _BV(ADSC);
#ifdef SAMPPIN
  digitalWrite(SAMPPIN, LOW);
#endif
...
ISR(ADC_vect)
{
  ...
  digitalWrite(SAMPPIN,HIGH);
  #endif
  // When ADLAR=0, ADCH has the 2 most signifant bits, ADCL has 8 bits
  result = ADCL;
  result |= ADCH<<8;
...
void calculateConstants(void)
{
  ...
  #ifdef CT4Phase
  /*  4 * (ADC_TIME uS) * (3.6e-4 deg / uS) * 50 Hz  - (degrees voltage ahead of current)*/
...
{
  // SAMPLERATE (360.0 / NUMSAMPLES) degrees
  // NUMSAMPLES is number of samples per 50 Hz cycle
